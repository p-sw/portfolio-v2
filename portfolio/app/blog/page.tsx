  ...
  const {data: posts, isLoading, error, refetch} = useQuery({
    ...
    queryFn: async (): Promise<Post[] | undefined | null> => {
      ...
      return apiData.items.map((item) => {
        console.log("map")
        return {...item, content: DomParser?.parseFromString(item.content, "text/html").querySelector("body")?.textContent?.slice(0, 500) ?? "Failed to parse content."}
      })
    },
    refetchOnWindowFocus: false
  })

  return <Flex direction={"column"} justify={"flex-start"} align={"center"} w={"full"} h={"calc(100vh - 80px)"} pt={10}>
    <Flex position={"sticky"} top={"80px"} left={0} zIndex={5} direction={"row"} w={"full"} h={"50px"} justify={"center"} align={"center"} bg={"transparent"} backdropFilter={"auto"} backdropBlur={"2px"}>
      <Flex bg={"background.1000"} direction={"row"} justify={"space-between"} align={"center"} p={1} borderRadius={"full"} w={"85%"} maxW={"800px"} h={"50px"} border={"1px solid"} borderColor={"reversebg.1000"}>
        ...
        <IconButton type={"submit"} icon={<BiSearch color={"var(--chakra-colors-wtext-1000)"} />} aria-label={"Search"} borderRadius={"full"} aspectRatio={1} bg={"primary.1000"} _hover={{bg: "primary.500"}} onClick={async () => {await refetch()}} />
      </Flex>
    </Flex>
    <Flex direction={"column"} justify={"flex-start"} align={"center"} w={"85%"} maxW={"800px"} h={"auto"} gap={3} pb={10} pt={5}>
      {
        function (): ReactNode {
          if (isLoading) {
            ...
            return <><Text>Error while searching posts.</Text></>
          } else if (posts.length <= 0) {
            return <><Text>Post Not Found.</Text></>
          } else {
            return posts.map((post, index) => {
              return <LinkBox key={index} w={"full"}>
