...
export async function GET(req: NextRequest, context: { params: { postid: string } }) {
  ...
  const post_res = await fetch(`https://www.googleapis.com/blogger/v3/blogs/${process.env.BLOGGER_BLOG_ID}/posts/${context.params.postid}?key=${process.env.BLOGGER_API_KEY}`)
  ...
  return NextResponse.json(post_data, {status: post_res.status, statusText: post_res.statusText})
}