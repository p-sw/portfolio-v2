...
import { SiTypescript, SiDjango, SiNextdotjs, SiTailwindcss, SiChakraui, SiFlask, SiFastapi, SiPm2 } from "react-icons/si"
...
function Category({title, children}: {title: string, children?: React.ReactNode}) {
  return <>
    <Heading className={`${categoryTitle.className}`} w={"90%"} maxW={"600px"} textAlign={["center", null, "left"]} mx={"auto"} fontSize={["2xl", null, "4xl"]}>{title}</Heading>
    <Flex direction={"row"} justify={["center", null, "flex-start"]} align={"center"} w={"90%"} maxW={"600px"} overflowX={"auto"} mx={"auto"} my={10} gap={5} pb={3}>
      {children}
    </Flex>
  </>
}
...
function CategoryIcon({name, children}: {name: string, children: As}) {
  return <Tooltip label={name}><Box as={"span"}><Icon as={children} w={12} h={12} /></Box></Tooltip>
}

export default function Skills() {
  return <Flex direction={"column"} justify={"center"} align={"flex-start"}>
