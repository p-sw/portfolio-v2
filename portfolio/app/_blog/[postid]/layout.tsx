...
export async function generateMetadata({
  ...
  const post_res = await fetch(
    `https://www.googleapis.com/blogger/v3/blogs/${process.env.BLOGGER_BLOG_ID}/posts/${params.postid}?key=${process.env.BLOGGER_API_KEY}`
  );
  ...
  return {
    ...
    description:
      new JSDOM(post_data.content).window.document
        .querySelector(`body`)
        ?.textContent?.slice(0, 100) ?? "Failed to parse post content.",
  };
}
